...
def create_lowest_cost_map(b_df):
    ...
    rawmaterial_indices = [8, 26]
    conversioncost_indices = [11, 29,32]
    logisticchoice_indices = [13]
    qualitycontrol_indices = [35, 38]
    
    for index, row in b_df.iterrows():
        for col_index in electronic_indices + mechanical_indices + rawmaterial_indices + conversioncost_indices+logisticchoice_indices+qualitycontrol_indices:
...
def process_quotes(a_df, b_df, max_quote_id):
    ...
    for quote_id in range(1, max_quote_id + 1):
        ...
        for _, a_row in quote_rows.iterrows():
            ...
            for col_index in rawmaterial_indices:
                if pd.notna(a_row.iloc[col_index]):
                    result = find_lowest_cost(a_row, lowest_cost_map, col_index, "Raw Material", q_col)
                    output_data.append(result)

            for col_index in conversioncost_indices:
                if pd.notna(a_row.iloc[col_index]):
                    result = find_lowest_cost(a_row, lowest_cost_map, col_index, "Conversion Cost", q_col)
                    output_data.append(result)

            for col_index in logisticchoice_indices:
                if pd.notna(a_row.iloc[col_index]):
...
output_df.to_csv('outputdraft.csv', index=False)
